...
export function TeamGoalCard({ current, goal, delay = 0, monthCurrent }: TeamGoalCardProps) {
  ...
  const dailyNeeded = useMemo(() => {
    if (!goal) return 0;
    const base = monthCurrent ?? current;
    const monthRemaining = Math.max(goal - base, 0);
    if (monthRemaining <= 0) return 0;
    const now = new Date();
    const lastDay = new Date(now.getFullYear(), now.getMonth() + 1, 0).getDate();
    const daysLeft = lastDay - now.getDate();
    if (daysLeft <= 0) return monthRemaining;
    return monthRemaining / daysLeft;
  }, [goal, current, monthCurrent]);
  ...
  return (
    ...
    >
      {tier !== "none" && tier !== "low" && (
        <div
          className={cn(
            "absolute inset-0 rounded-xl opacity-0 transition-opacity duration-1000 pointer-events-none",
            tier === "started" && "opacity-100",
            tier === "halfway" && "opacity-100",
            tier === "close" && "opacity-100",
            tier === "complete" && "opacity-100",
          )}
          style={{
            background:
              tier === "started"
      ...
      {tier === "complete" && (
        <div className="absolute inset-0 rounded-xl pointer-events-none overflow-hidden">
          <div
            ...
            style={{
              background:
                "linear-gradient(105deg, transparent 40%, hsla(43, 96%, 56%, 0.08) 45%, hsla(43, 96%, 56%, 0.15) 50%, hsla(43, 96%, 56%, 0.08) 55%, transparent 60%)",
      ...
      >
        <Target className={cn("w-6 h-6", tier === "complete" && "animate-pulse")} />
        {tier === "complete" && (
          <div className="absolute inset-0 rounded-lg bg-gold/10 animate-ping" style={{ animationDuration: "2s" }} />
        )}
      </div>

      <div className="min-w-0 flex-1 relative z-10">
        <p className="text-xs sm:text-sm text-muted-foreground font-medium">Meta del Equipo</p>
        {goal ? (
          <>
            <p
              className={cn(
            ...
            <div className="flex flex-wrap items-center justify-between gap-x-3 mt-1">
              <p
                className={cn(
                  ...
                  tier === "started" && "text-primary/70",
                  tier === "halfway" && "text-amber-500/70",
                  tier === "close" && "text-amber-400/80",
                  tier === "complete" && "text-gold/80",
                )}
              >
                {Math.round(pct)}% alcanzado
                {tier === "complete" && " 🎉"}
                {tier === "close" && " 🔥"}
              </p>
            ...
            {tier !== "complete" && dailyNeeded > 0 && (
              <p className="text-[10px] mt-0.5 text-muted-foreground/70">
                📅 Necesitan ~{formatCurrency(dailyNeeded)}/día para llegar a la meta
              </p>
            )}
          </>
        ) : (
